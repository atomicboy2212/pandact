function [DetVal rays] = ForwardProjection(x_s, y_s, x_d, y_d, N,image)
...
if ~isempty(cross)
   ...
   for j = 1:size(line_ind,1)-1
      ...
      %-------------add------------
       row = line_ind(j,2);
       col = line_ind(j,1);
       DetVal = DetVal + image(row,col)*v_lens(j);
       %------------------------------
   end;
end;
...
if( norm(rays'*image(:)-DetVal) > 1e-010 )
    disp(norm(rays'*image(:)-DetVal));
end;
  % disp(norm(rays'*image(:)-DetVal));